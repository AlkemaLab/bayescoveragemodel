...
assign_outliers <- function(dat,
  ...
  if (!is.null(outlier_record_ids)){
    ...
    dat <- dat |>
      dplyr::mutate(outlying_obs = ifelse (record_id_fixed %in% outlier_record_ids, 1, 0))
  } else {
    dat <- dat |>
      dplyr::mutate(outlying_obs = 0)
  }
  ...
  if ("possible_outlier" %in% names(dat)){
    if ("possible_outlier_userinput" %in% names(dat)){
      if (any(!is.na(dat$possible_outlier_userinput))){
        ...
        dat <- dat |>
          dplyr::mutate(nooutlier = 1 - ifelse(is.na(possible_outlier_userinput), possible_outlier, possible_outlier_userinput),
    ...
    if (any(is.na(dat$possible_outlier))){
      ...
      print("and outlying recordID, if provided")
      dat <- dat |>
        dplyr::mutate(nooutlier = 1 - possible_outlier,
               nooutlier = ifelse(outlying_obs == 1, 0, nooutlier))
      return(dat)
    }
  } else {
  ...
  dat <- dat |>
    dplyr::mutate(start_date = year) |>
    dplyr::mutate(isafter1990 = ifelse(start_date > 1990, 1, 0),
           yesoutlier = dplyr::case_when(
             outlying_obs == 1 ~ 1,
            any_bias == 1 ~ 1,
    ...
    dplyr::group_by(iso) |>
    dplyr::mutate(ndhs_nonoutlying = sum(data_series_type == "DHS" & !yesoutlier),
    ...
    dplyr::mutate(noreferenceyet = ifelse(sum(nooutlier) == 0 , 1, 0),
           most_recent_notoutlying = max(c(0, # just a small number added to avoid -Inf warnings
                                           start_date[yesoutlier == 0]))) |>
    dplyr::ungroup() |>
    dplyr::mutate(nooutlier = ifelse(noreferenceyet & start_date == most_recent_notoutlying, 1, nooutlier),
...
identify_outliers_in_global_fit <- function(fit){
  ...
  res2 <- tibble(c = fit$stan_data$geo_unit, t = fit$stan_data$time,
                 ...
                 y = fit$stan_data$y) |>
    dplyr::left_join(res) |>
    dplyr::mutate(error_y = y - inv_probit(eta))

  cutoff_y <- quantile(abs(res2$error_y), 0.9)
  res2 |>
    dplyr::filter(abs(error_y) > cutoff_y  | nooutlier == 0) |>
    dplyr::pull(record_id_fixed)
}
...
#     paste0(is_in_union, "_", seq(3001, 3001 + dplyr::n() -1))))
# dat$record_id_fixed
