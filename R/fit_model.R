...
fit_model <- function(
  ...
  stan_data <- list(
    ...
    N = nrow(data),
    S = nrow(source_index),
    held_out = array(data$held_out),
    isDHS = array(data$isDHS),
    nooutlier =array(data$nooutlier),
    any_bias = array(data$any_bias),
    time = array(data$t),
    t_min = array(t_min),
    t_max = array(t_max),
    # for geo_unit:
  ...
  if (!subnational){
    ...
    extra_stan_data <- c(extra_stan_data,
                         list(
                           ...
                           is_agg_obs = array(rep(0, dim(data)[1])),
                           n_agg_units = n_agg_units,
                           agg_unit = array(rep(0, dim(data)[1])),
