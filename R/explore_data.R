...
explore_data <- function(data, indicator_col, indicator_se_col = NULL, data_types_col = NULL, group_col, region_col = NULL, indicator_name = NULL, routine_data = NULL) {
  ...
  data <- data |>
    ...
    dplyr::mutate(
      ...
      rate_of_change = get(indicator_col) - dplyr::lag(get(indicator_col)),
  ...
  if (!is.null(routine_data)) {
    country_codes <- data |> dplyr::pull(CODE_ISO_NUMERIC) |> unique()
    ...
    routine_data <- routine_data |>
      dplyr::filter(CODE_ISO_NUMERIC %in% country_codes) |>
      dplyr::left_join(country_names, by = "CODE_ISO_NUMERIC") |> # Join to add the country column from country_names
      dplyr::select(country = country.y, dplyr::everything(), -country.x) |>
      dplyr::arrange(year) |>
      dplyr::group_by(dplyr::across(dplyr::all_of(group_col))) |>
      dplyr::mutate(
        year_diff = year - dplyr::lag(year),
        routine_rate_of_change = routine_value - dplyr::lag(routine_value)
      ) |>
      dplyr::ungroup()

    routine_country_codes <- routine_data |> dplyr::pull(CODE_ISO_NUMERIC) |> unique()

    data <- data |>
      dplyr::filter(CODE_ISO_NUMERIC %in% routine_country_codes)
  }
  ...
  p1 <- ggplot2::ggplot(data, aes_mapping) +
    ggplot2::geom_point()

  # Add error bars if indicator_se_col is provided
  if (!is.null(indicator_se_col)) {
    p1 <- p1 +
      ggplot2::geom_errorbar(ggplot2::aes(ymin = indicator_level - get(indicator_se_col), ymax = indicator_level + get(indicator_se_col)),
                    alpha = 0.6, width = 0.3)
  }
  ...
  if (!is.null(routine_data)) {
    p1 <- p1 +
      ggplot2::geom_point(data = routine_data, ggplot2::aes(x = year, y = routine_value, colour = "Routine data"))
  }
  ...
  p2 <- ggplot2::ggplot(data, aes_mapping) +
    ggplot2::geom_point() +
    ggplot2::labs(
  ...
  if (!is.null(routine_data)) {
    p2 <- p2 +
      ggplot2::geom_point(data = routine_data, ggplot2::aes(x = year, y = routine_rate_of_change, colour = "Routine data"))
  }
  ...
  p3 <- ggplot2::ggplot(data |> dplyr::filter(!is.na(annualised_rate_of_change)), aes_mapping) +
    ggplot2::geom_point() +
    ggplot2::labs(
    ...
    ggplot2::facet_wrap(facet_formula)+
    ggplot2::geom_smooth(ggplot2::aes(group = 1)) #+
    #ggplot2::geom_smooth(method.args = list(family = "symmetric", span = 2), se = FALSE)

  # If routine data exists, add it to the rate of change vs level plot
  if (!is.null(routine_data)) {
    p3 <- p3 +
      ggplot2::geom_point(data = routine_data, ggplot2::aes(x = routine_value, y = routine_rate_of_change, colour = "Routine data"))  # Customize as needed
  }
